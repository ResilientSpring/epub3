...
class Ebook(ArchiveBookItem):
    ...
    def __init__(self, item_dir, item_identifier, item_bookpath, debug=False, args=False):
        ...
        self.abbyy_file = "{tmp}/{item_identifier}_abbyy".format(
            tmp=self.tmpdir.name, item_identifier=self.item_identifier)
        self.logger.debug("Temp directory: {}\nidentifier: {}".format(
            self.tmpdir.name, self.item_identifier))
    ...
    def extract_images(self):
        ...
        cover_file = "{tmp}/{item_bookpath}_jp2/{item_bookpath}_0001.jp2".format(
            tmp=self.tmpdir.name, item_bookpath=self.item_bookpath
        )
    ...
    def make_image(self, block):
        ...
        try:
            ...
            raise Exception("Can't open image {}: {}".format(origfile, e))
        try:
    ...
    def craft_epub(self, epub_outfile="out.epub"):
        ...
        epub_outfile = epub_outfile if epub_outfile.endswith('.epub') else '%s.epub' % epub_outfile
